...
namespace SMS.Data.Services
{
    public class StudentService : IStudentService
    {
        ...
        {
            // try to find a user with same name. If found return null (duplicates not allowed)
            var user = db.Users.FirstOrDefault(o => o.Username == username);

            // if user is unique store in database
            if (user!=null)
            {
               return null;
            }
            
            var u = new User {Username = username, Password = password, Role = role};
